...
function AnalyticsModal({ onClose }) {
  ...
  useEffect(() => {
    const weekly = storage.getWeeklyAnalytics();
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: '700px' }} onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h3 className="modal-title"><TrendingUp size={20} /> Discipline Analytics</h3>
        ...
        <div className="modal-body">
          ...
          <div className="tabs" style={{ marginBottom: '1rem' }}>
            ...
            <button className={`tab ${tab === 'heatmap' ? 'active' : ''}`} onClick={() => setTab('heatmap')}>Monthly Heatmap</button>
          </div>
          ...
          {tab === 'weekly' && (
            <div>
              ...
              <div style={{ height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData}>
                    ...
                    <Tooltip formatter={(value) => [`${value}%`, 'Score']} contentStyle={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '0.5rem' }} />
              ...
              <div style={{ marginTop: '1rem' }}>
                <h4 style={{ fontSize: '0.875rem', fontWeight: 600, marginBottom: '1rem' }}>Task Completion Trend</h4>
                <div style={{ height: 150 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={weeklyData}>
                      <XAxis dataKey="day" tick={{ fontSize: 12 }} />
                      <YAxis tick={{ fontSize: 12 }} />
                      <Tooltip formatter={(value, name) => [value, name === 'completed' ? 'Completed' : 'Total']} contentStyle={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '0.5rem' }} />
            ...
            <div>
              ...
              <div className="heatmap">
                {heatmapData.map((day, i) => (
                  <div key={i} className="heatmap-cell" data-level={day.level} title={`${day.date}: ${day.score}%`} />
                ))}
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '0.75rem', alignItems: 'center', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                <span>Less</span>
                {[0, 1, 2, 3, 4].map(level => (
                  <div key={level} className="heatmap-cell" data-level={level} style={{ width: 12, height: 12 }} />
                ))}
                <span>More</span>
              </div>
            </div>
